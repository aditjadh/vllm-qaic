...
# ── Configuration ────────────────────────────────────────────────────────────
# moe_mmk is a @triton.jit device function (returns an accumulator), called from
# expert_triton_kernel. We launch expert_triton_kernel (single program, single
# expert) on the unquantized path: C = A @ B, A[M,K] @ B[K,N].
M       = 16
N       = 64
K       = 128
DTYPE   = torch.float16
...
BLOCK_M = 16            # must be >= M for the single-program kernel
BLOCK_N = 64
BLOCK_K = 64

KERNEL_NAME = "moe_mmk (via expert_triton_kernel)"
LOG_DIR     = os.path.join(os.path.dirname(__file__), "..", "logs")
os.makedirs(LOG_DIR, exist_ok=True)
if "MOEMMK_LOG_PATH" in os.environ:
    log_path  = os.environ["MOEMMK_LOG_PATH"]
    timestamp = os.path.basename(log_path)[len("test_moe_mmk_"):-len(".log")]
else:
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    log_path  = os.path.join(LOG_DIR, f"test_moe_mmk_{timestamp}.log")
...
def main():
    ...
    with open(log_path, "w") as f:
        try:
            ...
            log(f, "  quant: unquantized (use_fp8_w8a8=False, use_int8_w8a16=False)")
            ...
            # Force a device sync so any kernel compile/exec failure surfaces here.
            c_host = c.cpu()
...
if __name__ == "__main__":
    # Child role: run the kernel.
    if os.environ.get("MOEMMK_CHILD") == "1":
        main()
        sys.exit(0)
    ...
    env = dict(os.environ, MOEMMK_CHILD="1", MOEMMK_LOG_PATH=log_path)
    ...
    # (negative return code). A positive code means main() already logged it.
