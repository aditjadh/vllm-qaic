...
DEVICE = "qaic"
KERNEL_NAME = "l2norm_fwd_kernel2"
...
def main(log_path):
    ...
    with open(log_path, "w") as f:
        ...
        try:
            import os as _os
            # USE_DEFAULT_FLA_NORM=0 (default) => l2norm_fwd_kernel2 (MBLOCK-tiled)
            _os.environ["USE_DEFAULT_FLA_NORM"] = "0"
            ...
            log(f"  USE_DEFAULT_FLA_NORM=0 => l2norm_fwd_kernel2")
            log("")

            MBLOCK = 32
            import math
            grid_m = math.ceil(T / MBLOCK)
            log("Grid Configuration:")
            log(f"  grid: ({grid_m},)")
            log(f"  MBLOCK={MBLOCK}, N={D}")
            log("")
...
if __name__ == "__main__":
    run_test(main, LOG_DIR, "l2norm_fwd_kernel2")
