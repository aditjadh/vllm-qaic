...
DEVICE = "qaic"
KERNEL_NAME = "layer_norm_gated_fwd_kernel1"
...
def main(log_path):
    ...
    with open(log_path, "w") as f:
        ...
        try:
            ...
            # D > 512 triggers layer_norm_gated_fwd_kernel1 (per-row variant)
            T, D = 16, 1024
            ...
            log(f"  (D>512 => layer_norm_gated_fwd_kernel1, grid=({T},))")
            log("")

            log("Grid Configuration:")
            log(f"  grid: ({T},)")
            log(f"  BD=next_power_of_2({D})=1024")
            log("")
...
if __name__ == "__main__":
    run_test(main, LOG_DIR, "layer_norm_gated_fwd_kernel1")
